...
def calcuSlack(myGraph, travelPath, node, insertLocation, timeParamDict):
    ...
    # if insertLocation == 1:
    #     print(node['ID'], timeParamDict['arriveTimeToCandiNode'])
...
def toptw(startP, destP, myGraph, day):
    travelTime = 0
    travelPath = [{'ID':startP}, {'ID':destP}]
    totalProfit = 0
    bestRatio = -1
    # 用来记录加入到路径中的点
    existList = [startP, destP]
    ...
    while finish == False:
        ...
        # print('bestkpath', len(bestKPath))
        # print(bestKPath[selectNode])
